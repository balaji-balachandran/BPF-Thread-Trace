...
{   
    ...
    if (args->family == AF_INET)
    {
        ...
        if (packet_value != 0)
        {
            bpf_trace_printk("Got packet from connection\n src_ip = %ld\n", packet_value);
...
// This runs when attaching the kprobe
int trace_connect_entry(struct pt_regs *ctx, struct sock *sk)
{
    ...
    // This is the pid of the process that is connecting
